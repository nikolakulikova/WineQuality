...
def final_predict(X_train, y_train, X_test, y_test):
    models = [{"model": RandomForestClassifier(n_estimators=64, max_depth=7), "name": "RFC"},
              ...
              {"model": KNeighborsClassifier(n_neighbors=67), "name": "KNC"}]

    print("Baseline for train: {}".format(baseline(y_train) / len(y_train)))
    print("Baseline for test: {}".format(baseline(y_test) /  len(y_test)))
    for model in models:
